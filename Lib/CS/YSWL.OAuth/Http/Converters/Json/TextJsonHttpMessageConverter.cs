...
namespace YSWL.OAuth.Http.Converters.Json
{
    /// <summary>
    ...
    public class TextJsonHttpMessageConverter : AbstractHttpMessageConverter
    {
        ...
        /// <summary>
        /// Creates a new instance of the <see cref="TextJsonHttpMessageConverter"/> 
        /// with the media type 'ALL'.
        /// </summary>
        public TextJsonHttpMessageConverter() :
            this(new JsonMapper())
        {
        }
        ...
        public TextJsonHttpMessageConverter(JsonMapper mapper) :
            base(MediaType.ALL)
        {
            this.mapper = mapper;
        }
        ...
        protected override T ReadInternal<T>(IHttpInputMessage message)
        {
            ...
            using (StreamReader reader = new StreamReader(message.Body, encoding))
            {
                ...
                // Parse JSON
                JsonValue jsonValue = JsonValue.Parse(ConvertToJson(result));
        ...
        protected virtual string ConvertToJson(string result)
        {
            ...
            foreach (string item in result.Split('&'))
            {
                ...
                {
                    jsonStr.Append("\"");
                    jsonStr.Append(split[0]);
                    jsonStr.Append("\"");
                    jsonStr.Append(":");
                    jsonStr.Append("\"");
                    jsonStr.Append(split[1]);
                    jsonStr.Append("\"");
                    jsonStr.Append(",");
                }
            }
