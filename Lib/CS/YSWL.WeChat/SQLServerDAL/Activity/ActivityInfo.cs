...
namespace YSWL.WeChat.SQLServerDAL.Activity
{
	...
	public partial class ActivityInfo:IActivityInfo
	{
        ...
        public int GetMaxId()
        {
            return DBHelper.DefaultDBHelper.GetMaxID("ActivityId", "WeChat_ActivityInfo");
        }
        ...
        public YSWL.WeChat.Model.Activity.ActivityInfo GetActivity(int activityId, int type)
        {
            ...
            strSql.Append(" where Status=1 and Type=@Type and StartDate<=GETDATE() and  EndDate>=GETDATE()");
            if (activityId > 0)
            {
