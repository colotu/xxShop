namespace YSWL.Payment.PaymentInterface.WeChat.Utils
{
    ...
    public class WxPayHelper
    {
        ...
        public string CreateDeliverNotifyXml(Dictionary<string, string> bizObj)
        {
            bizObj.Add("appid", AppId);
            bizObj.Add("deliver_timestamp",
                ((DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000000).ToString());
            bizObj.Add("app_signature", GetBizSign(bizObj));
            bizObj.Add("sign_method", SignType);

            var entries = bizObj.Select(d => string.Format("\"{0}\": \"{1}\"", d.Key, d.Value));
            return "{" + string.Join(",", entries.ToArray()) + "}";
        }
        ...
        public string CreateNativeUrl(string productid)
        {
            ...
            try
            {
                Dictionary<string, string> nativeObj = new Dictionary<string, string>();
                nativeObj.Add("appid", AppId);
                nativeObj.Add("productid", HttpUtility.UrlEncode(productid, Encoding.GetEncoding("UTF-8")));
                nativeObj.Add("timestamp", ((DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000000).ToString());
                nativeObj.Add("noncestr", CommonUtil.CreateNoncestr());
