...
;
(function () {
    var currenScript = $('[src$="maticsoft.SelectTaoBaoNode.js"]').last(); //锁定脚本引用
    ...
    function InitLastSelect() {
        ...
        $.ajax({
            ...
            timeout: 10000,
            async: false,
            data: { Action: "GetTaoBaoParentNode", NodeId: $('[HFID=' + currentBaseSelectGuid + ']').val() },
    ...
    function InitFirstSelect() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            timeout: 10000,
            data: { Action: "GetTaoBaoDepthNode" },
            async: false,
            success: function (resultData) {
                switch (resultData.STATUS) {
                    case "OK":
    ...
    function LoadChildNodeData(send) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            timeout: 10000,
            data: { Action: "GetTaoBaoChildNode", ParentId: $(send).val() },
            async: false,
            success: function (resultData) {
                switch (resultData.STATUS) {
                    case "OK":
                        ...
                        $(resultData.DATA).each(function () {
                            ...
                            for (var key in this) {
                                ...
                                if (index == 2) {//根据字段的index判断，万一数据库字段顺序变了就会造成数据不对。需要改善
                                    name = this[key];
                                }
                                index++;
                            }
