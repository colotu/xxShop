...
namespace YSWL.WeChat.BLL.Core
{
	...
	{
        private readonly INoReplyMsg dal = YSWL.DBUtility.PubConstant.IsSQLServer ? (INoReplyMsg)new WeChat.SQLServerDAL.Core.NoReplyMsg() : (INoReplyMsg)new WeChat.MySqlDAL.Core.NoReplyMsg();
		public NoReplyMsg()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.WeChat.Model.Core.NoReplyMsg GetModelByCache(long MsgId)
		{
			...
			return (YSWL.WeChat.Model.Core.NoReplyMsg)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.WeChat.Model.Core.NoReplyMsg> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.WeChat.Model.Core.NoReplyMsg> DataTableToList(DataTable dt)
		{
			List<YSWL.WeChat.Model.Core.NoReplyMsg> modelList = new List<YSWL.WeChat.Model.Core.NoReplyMsg>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.WeChat.Model.Core.NoReplyMsg model;
        ...
        /// <returns></returns>
        public long AddMsg(YSWL.WeChat.Model.Core.RequestMsg msg)
        {
        ...
        {
            return dal.GetList(top, status,userName, startdate, enddate, keyword, filedOrder);
        }
