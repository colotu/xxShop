...
namespace YSWL.MALL.SQLServerDAL.Shop.PromoteSales
{
	...
	public partial class CountDown:ICountDown
	{
		...
		public int GetMaxId()
		{
		return DBHelper.DefaultDBHelper.GetMaxID("CountDownId", "Shop_CountDown"); 
		}
		...
		public bool Exists(int CountDownId)
		{
			...
			SqlParameter[] parameters = {
					new SqlParameter("@CountDownId", SqlDbType.Int,4)
			};
		...
		public YSWL.MALL.Model.Shop.PromoteSales.CountDown GetModel(int CountDownId)
		{
			...
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select  top 1 CountDownId,ProductId,EndDate,Description,Sequence,Price,Status,LimitQty  from Shop_CountDown ");
			strSql.Append(" where CountDownId=@CountDownId");
			SqlParameter[] parameters = {
					new SqlParameter("@CountDownId", SqlDbType.Int,4)
			};
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from Shop_CountDown T ");
	    ...
	    {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 *  from Shop_CountDown ");
            strSql.Append(" where ProductId=@ProductId and Status=1 and EndDate>getdate() and  ");
            strSql.Append("EXISTS( SELECT *  FROM PMS_Products WHERE SaleStatus=1 AND ProductId=Shop_CountDown.ProductId )  ");
            SqlParameter[] parameters = {
					new SqlParameter("@ProductId", SqlDbType.BigInt)
			};
            parameters[0].Value = ProductId;
