    ...
    public static class PinyinHelper
    {
        ...
        static public string GetPinyin(string chineseChars)
        {
            ...
            for (int i = 0; i < byteArray.Length; i++)
            {
                ...
                else if ((byteValue >= 48 && byteValue <= 57) || (byteValue >= 65 && byteValue <= 90) || (byteValue >= 97 && byteValue <= 122))
                {
            ...
            string str= sb.ToString().Replace("--","-").Replace("--","-").ToLower();
            if (str.Replace("--", "-").LastIndexOf("-") == str.Replace("--", "-").Length - 1)
            {
        ...
        {
            chineseChars = Common.StringPlus.SubString(chineseChars.Replace("'", "").Replace("‘", "").Replace("\"", "").Replace("“", "").Replace("”", "").Replace(" ", "").Replace(",", "").Replace("，", ""), 15);
            byte[] byteArray = System.Text.Encoding.Default.GetBytes(chineseChars);
            int byteValue;
            StringBuilder sb = new StringBuilder(chineseChars.Length * 4);
            for (int i = 0; i < byteArray.Length; i++)
