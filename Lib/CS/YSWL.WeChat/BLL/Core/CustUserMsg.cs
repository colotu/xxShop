...
namespace YSWL.WeChat.BLL.Core
{
	...
	{
        private readonly ICustUserMsg dal = YSWL.DBUtility.PubConstant.IsSQLServer ? (ICustUserMsg)new WeChat.SQLServerDAL.Core.CustUserMsg() : (ICustUserMsg)new WeChat.MySqlDAL.Core.CustUserMsg();
		public CustUserMsg()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.WeChat.Model.Core.CustUserMsg GetModelByCache(long MsgId,string UserName)
		{
			...
			return (YSWL.WeChat.Model.Core.CustUserMsg)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.WeChat.Model.Core.CustUserMsg> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.WeChat.Model.Core.CustUserMsg> DataTableToList(DataTable dt)
		{
			List<YSWL.WeChat.Model.Core.CustUserMsg> modelList = new List<YSWL.WeChat.Model.Core.CustUserMsg>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.WeChat.Model.Core.CustUserMsg model;
