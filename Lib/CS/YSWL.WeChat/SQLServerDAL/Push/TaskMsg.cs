...
namespace YSWL.WeChat.SQLServerDAL.Push
{
	...
	public partial class TaskMsg:ITaskMsg
	{
        ...
        public int GetMaxId()
        {
            return DBHelper.DefaultDBHelper.GetMaxID("TaskId", "WeChat_TaskMsg");
        }
         ...
         public DataSet GetMsgList(string openid, string datetime)
        {
            ...
            strSql.Append("select * ");
            strSql.Append(" FROM WeChat_TaskMsg ");
            strSql.AppendFormat("  where OpenId='{0}' and PublishDate BETWEEN '{1}' AND GETDATE()", Common.InjectionFilter.SqlFilter(openid), Common.InjectionFilter.SqlFilter(datetime));
            return DBHelper.DefaultDBHelper.Query(strSql.ToString());    
        }
         public DataSet GetList(int top, string openId, string startdate, string enddate, string filedOrder)
         {
             ...
             if (!String.IsNullOrWhiteSpace(enddate) && Common.PageValidate.IsDateTime(enddate))
             {
                 ...
                 strWhere.AppendFormat("  CreatedDate< dateadd(day,1,'{0}')", enddate);
             }
