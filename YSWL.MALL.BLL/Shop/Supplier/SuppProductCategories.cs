...
namespace YSWL.MALL.BLL.Shop.Supplier
{
	...
	public partial class SuppProductCategories
	{
		...
		{
			return dal.Exists(CategoryId,ProductId);
		}
		...
		public YSWL.MALL.Model.Shop.Supplier.SuppProductCategories GetModel(int CategoryId,long ProductId)
		{
			...
			return dal.GetModel(CategoryId,ProductId);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public YSWL.MALL.Model.Shop.Supplier.SuppProductCategories GetModelByCache(int CategoryId,long ProductId)
		{
			...
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(CategoryId,ProductId);
					if (objModel != null)
					{
			...
			return (YSWL.MALL.Model.Shop.Supplier.SuppProductCategories)objModel;
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.Shop.Supplier.SuppProductCategories> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<YSWL.MALL.Model.Shop.Supplier.SuppProductCategories> DataTableToList(DataTable dt)
		{
			List<YSWL.MALL.Model.Shop.Supplier.SuppProductCategories> modelList = new List<YSWL.MALL.Model.Shop.Supplier.SuppProductCategories>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				YSWL.MALL.Model.Shop.Supplier.SuppProductCategories model;
        ...
        /// 得到一个对象实体，从缓存中
        /// </summary>
        public YSWL.MALL.Model.Shop.Supplier.SuppProductCategories GetModelByCache(long ProductId)
        {
            string CacheKey = "SuppProductCategoriesModel-Prod-" + ProductId;
            object objModel = YSWL.Common.DataCache.GetCache(CacheKey);
            if (objModel == null)
            {
                try
