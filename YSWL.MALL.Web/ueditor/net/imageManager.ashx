...
public class imageManager : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        string path = context.Server.MapPath("~/Upload/RTF/" + "/");//+ DateTime.Now.ToString("yyyyMMdd") + "/");                  //最好使用缩略图地址，否则当网速慢时可能会造成严重的延时
        ...
        if(action == "get")
        {
            ...
            if (info.Exists)
            {
                foreach (FileInfo fi in info.GetFiles("*", SearchOption.AllDirectories))
                {
                    ...
                    {
                        str += "Upload/RTF/" +fi.Directory.Name+ "/" + fi.Name + "ue_separate_ue";
                    }
                }
            }
