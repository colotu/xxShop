...
function rowClicked(obj)
{
  ...
  function submitAllValue(objTextRegion) {
      var subCheckBoxList = new Array();
      ...
      for (var i = 0; i < subCheckBoxList.length; i++) {
          if (subCheckBoxList[i].checked) {
              inputTextValue[i] = subCheckBoxList[i].value;
              inputIDValue[i] = subCheckBoxList[i].id;
          }
      }
      for (var l = 0; l < inputTextValue.length; l++) {
          if (inputTextValue[l] != null) {
              inputTempValue = inputTempValue.concat(inputTextValue[l]);
              inputTempIDValue = inputTempIDValue.concat(inputIDValue[l]);
          }
      }

      //var thisID=$_getID(objTextRegion);
      var thisID = objTextRegion
      var thisIDValue = $_getID(thisID.id + "_Id");
      // var thisID=$_getID("CreateShippingMode_shippingModeView_txtRegion");
      // var thisIDValue=$_getID("CreateShippingMode_shippingModeView_txtRegion_Id");
      //var  thisID = $_getID("OK").className;
      //var  hisIDValue = $_getID("OK").className + "_id";
      ...
      /*
      thisID = $_getID("OK").className;
      thisIDValue = $_getID("OK").className + "_id";
      //alert(inputTempValue[0].id);
      if (thisID == "")
      {
      return false;
      }
      ...
      {
      $_getID(thisID).value = inputTempValue.toString();
      $_getID(thisIDValue).value = inputTempIDValue.toString();
