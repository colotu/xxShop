...
;
(function() {
    var currenScript = $('[src$="maticsoft.selectregion.delivery.js"]').last(); //锁定脚本引用
    var currentBaseSelectGuid = $.Guid.New(); //生成闭包GUID 唯一标识
    //给隐藏域设置GUID属性 以便闭包内脚本跟踪
   var currhfSelectedNode = $('[id$=hfSelectedNode]').last().attr('HFID', currentBaseSelectGuid);
            ...
            success: function(resultData) {
                ...
                switch(pagename){
                    ...
                    case'cart_list'://购物车列表页
                        setCartListDeliveryAreas($(send).val());
                         break;
                    default:
                         break;
                }
