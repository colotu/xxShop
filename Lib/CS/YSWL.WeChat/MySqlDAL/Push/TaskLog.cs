...
namespace YSWL.WeChat.MySqlDAL.Push
{
    ...
    public partial class TaskLog : ITaskLog
    {
        ...
        public int GetMaxId()
        {
            return DbHelperMySQL.GetMaxID("TaskId", "WeChat_TaskLog");
        }
        ...
        public YSWL.WeChat.Model.Push.TaskLog GetModel(int TaskId, string UserName, string tableName)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select TaskId,UserName,CreatedTime from  " + tableName);
                strSql.Append(" where TaskId=?TaskId and UserName=?UserName LIMIT 1 ");
                MySqlParameter[] parameters = {
					new MySqlParameter("?TaskId", MySqlDbType.Int32,4),
					new MySqlParameter("?UserName", MySqlDbType.VarChar,200)			};
                parameters[0].Value = TaskId;
